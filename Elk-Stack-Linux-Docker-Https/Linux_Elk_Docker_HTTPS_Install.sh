...
#
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - pull the dockers"
sudo docker-compose pull #gets the images
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - compose up - To Create volume"
sudo docker-compose up -d #Brings up to create volumes if you don't do it yourself
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - compose down - Because we don't need it up"
sudo docker-compose down #They go down because I only needed the volumes created
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - create_certs - This is to inject certificates for https stuff"
sudo docker-compose -f create-certs.yml run --rm create_certs #This pulls certs so they can boot up
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - compose up - Doesn't have to be up but it makes me feel better"
sudo docker-compose up -d #es01 and kib01 should be up now as a site
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - Steal Certificate ca and bring it locally to secure connections later"
sudo docker cp es01:/usr/share/elasticsearch/config/certificates/ca/ca.crt ./ca.crt #This Steals certs so you can use for winlogbeats,filebeat,logstash
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - I dont know, make a copy of the certs to be safe"
sudo cp ./ca.crt ./ca.crt.bak #make a copy to main folder
#(( sc++ )) || true && $ts ${sc}; echo "EchoCount '${ec}' - Go Metabee! - Make Another copy for filebeats"
#sudo cp ./ca.crt /etc/filebeat/elastic.crt #Shove one in the filebeat folder
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - compose up - This should bring Kib01 up while es01 should already be up"
sudo docker restart kib01 #Feels like I should restart kib
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - Make Passwords - For Elastic and Kiabana and such"
...
#Once you get your passwords compose down and replace the new password for kibana system in docker-compose.yml
    (( sc++ )) || true && $ts "${sc}"; echo "EchoCount '${ec}' - Go Metabee! - compose down - This brings everything down so you can change file data"
sudo docker-compose down
(( sc++ )) || true && echo " Go Metabee! - I Paused this for 120 Seconds - HURRY Go replace docker-compose.yml with the new password you just got!!"
(( sc++ )) || true && echo " Go Metabee! - I Paused this for 120 Seconds - HURRY Go replace docker-compose.yml with the new password you just got!!"
(( sc++ )) || true && echo " Go Metabee! - I Paused this for 120 Seconds - HURRY Go replace docker-compose.yml with the new password you just got!!"
(( sc++ )) || true && echo " Go Metabee! - I Paused this for 120 Seconds - HURRY Go replace docker-compose.yml with the new password you just got!!"
sleep 110
(( sc++ )) || true && echo " Go Metabee! - Starting Back Up in 10 Seconds"
(( sc++ )) || true && echo " Go Metabee! - Starting Back Up in 10 Seconds"
(( sc++ )) || true && echo " Go Metabee! - Starting Back Up in 10 Seconds"
(( sc++ )) || true && echo " Go Metabee! - Starting Back Up in 10 Seconds"
sleep 10
(( sc++ )) || true && echo " Go Metabee! - Hopefully you are done..."
(( sc++ )) || true && echo " Go Metabee! - Hopefully you are done..."
(( sc++ )) || true && echo " Go Metabee! - Hopefully you are done..."
sleep 2
    #(( sc++ )) || true && $ts ${sc}; echo "EchoCount '${ec}' - Go Metabee! - Bring Sally up and Navigate to it"
    #You Raise Me Up(8)
(( sc++ )) || true && echo " Go Metabee! - Now that this comes up, you should be done. Go to https://localhost:5601 ..."
